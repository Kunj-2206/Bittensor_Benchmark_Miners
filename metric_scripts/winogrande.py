...
def evaluate_models_winogrande(data, datetime_folder):
    
    dataset = load_dataset("winogrande", "winogrande_debiased")
    dataset = dataset['train']
    # Access individual columns from the dataset
    sentences = dataset['sentence'][:data]
    option1 = dataset['option1'][:data]
    option2 = dataset['option2'][:data]
    ...
    try:
        ...
        for sentence, option1, option2, answer in zip(sentences, option1, option2, answers):
            ...
            system_prompt = "You are an AI assistant. Your task is to reply correct option which can fill the blank, from given text and remember only answer in one character like (1,2), no other text"
            ...
            for response in model_response['choices']:
                ...
                if str(answer) in str(extract_selected_option(model_resp, option1, option2)):
                    model_score = 3
                    model_accuracy = 1
                else:
                    model_score = 0
            ...
            if que_no%100 == 0:
                    with open(f'checkpoints/{datetime_folder}/winogrande/Winogrande_Miner_Metrics_{que_no}.json', 'w') as chkpnt_file:
                        chkpnt_file.write(json.dumps(miner_scores))
                        
            que_no += 1
            
        with open(f'results/{datetime_folder}/Winogrande_Miner_Metrics.json', 'w') as json_file:
        ...
        print()
        traceback.print_exc()
        with open(f'except/{datetime_folder}/Winogrande_Miner_Metrics_{que_no}.json', 'w') as json_file:
            json_file.write(json.dumps(miner_scores))
